...
#                 comm.distance = cal.S2Bgenes.values(g, rs1,rs2, allg)  #!!!!!!!!!!!!!
#                 return(comm.distance)
#             })
#             comm.distance.list = do.call(cbind, comm.distance.list) %>% 'rownames<-'(allg)
#             return(comm.distance.list)
...
#             intg = edges$res[ixix] %>% lapply(function(xx) names(xx[2:(length(xx)-1)]))
...
# cal.S2Bgenes.values <- function(g, community1, community2, allg){
    
#     scorev <- rep(0, length(allg))
#     names(scorev) <- allg
